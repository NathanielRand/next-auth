...
You may notice we are using environment variables in the code example above. We take the value of `GITHUB_ID` and `GITHUB_SECRET` from the GitHub Developer OAuth Portal. See [Configuring OAuth Provider](/getting-started/providers/oauth-tutorial#2-configuring-oauth-provider) section on how to get those.
...
Auth.js is fully customizable - [our guides section](/guides) teaches you how to set it up to handle auth in different ways. All the possible configuration options are [listed here](/reference/core#authconfig).
:::
...
For more information please check out our [deployment page](/getting-started/deployment).
